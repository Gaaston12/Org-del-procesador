...
;21-10-16
;PROGRAMA: Modificación del asm_io_asm para agregarle la función prinf_hex para mostrar números en hexa
; file: asm_io.asm
...
hex_format 		db  "%X", 0 	;Para imprimir en hexadecimal, mayúsculas
...
%endif
	...
	global 	print_hex	;Agrego la subrutina print_hex
        global  sub_dump_math, sub_dump_stack
        extern  _scanf, _printf, _getchar, _putchar

read_int:	;IMPORTANTE: Recordar que en C, el scanf se llama así: scanf("%i",&num), donde el ampersam pasa la direccion de m
			;memoria
	enter	4,0		;Es lo mismo que hacer:	 push ebp  		//Para almacenar el ebp viejo
											;mov ebp, esp   //Colocar la base donde está marcando el esp
											;sub esp, 4		//Para almacenar el espacio para una variable
	pusha 						;Para cargar los regsitros en la pila
	pushf 						;Para cargar los flags en la pila

	lea	eax, [ebp-4] 			;Lea es como la desreferenciación, es como poner [[ebp-4]]. Se está guardando en eax
								;la dirección de memoria de ebp-4, que es donde se va a almacenar el entero. Este es uno 
								;de los parátros que necesita el scanf: &num
								
	push	eax   				;Carga en la pila, la dirección de memoria de donde se va a almacenar el entero
	push	dword int_format  	;Carga en al pila la cadena "%i",0  .Este es uno de los parámetros de scanf
	call	_scanf 				;Llama a la función scanf
	pop	ecx 					;Saca un elemento de la pila y lo guarda en ecx. Se guarda el parámetro "%i"
	pop	ecx 					;Saca un elemento de la pila y lo guarda en ecx. Se guarda la dirección de memoria de 
								;donde está el parámetro, pero no el valor en sí
	
	popf						;Devuelve el valor de los flags
	popa						;Devuelve el valor de los registros
	mov	eax, [ebp-4]			;Guarda en eax el valor que se ingresó por el teclado
	leave						; mov esp, ebp 		// Para eliminar los espacios de las variables locales
								; pop ebp 			// Para colocar el ebp donde estaba anteriormente

	ret 						;Retorna el IP a la dirección de la instrucción siguiente 

print_int:	;IMPORTANTE: Recordar que en C, el printf se suele llamar así: printf("s",expresión)
	enter	0,0					
	pusha
	pushf

	push	eax 				;Coloca eax en la pila. Esto se debe a que en eax nosotros almacenamos el 
								;entero y luego llamamos a esta función.
	push	dword int_format 	;Coloca en la pila la cadena "%i"
	call	_printf				;Llama a la función printf de C
	pop	ecx                     ;Coloca en ecx uno de los parámetros
	pop	ecx 					;Coloca en ecx el parámetro "i"

	popf 						;Devuelve el valor de los flags
	popa 						;Devuelve el valor de los registros
	leave						; mov esp, ebp 		// Para eliminar los espacios de las variables locales
								; pop ebp 			// Para colocar el ebp donde estaba anteriormente
	ret 						;Para retornar el esp donde estaba
